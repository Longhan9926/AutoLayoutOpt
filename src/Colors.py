...
def generate_color_palette(n, dominant_color, strategy='Monotone'):
    ...
    dominant_color = colorsys.rgb_to_hsv(dominant_color[0], dominant_color[1],
                                         dominant_color[2])  # convert the dominant color to hsv space
    prime_hue = dominant_color[0]
    prime_sat = dominant_color[1]
    prime_val = dominant_color[2]
    ...
    while len(color_palette) < n:
        ...
        if choice == 'hue':
            if strategy == 'Monotone':
                temp_hue = np.random.normal(loc=prime_hue, scale=0.2, size=None)
                ...
                temp_hue = temp_hue - math.floor(temp_hue)
            temp_sat = random_sat(prime_sat, 0.5)
